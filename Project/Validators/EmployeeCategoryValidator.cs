...
{
    public class EmployeeCategoryValidator : BaseUniqueValidator<EmployeeCategoryDto, EmployeeCategory>
    {
        private readonly IEmployeeCategoryRepository _repository;

        public EmployeeCategoryValidator(IEmployeeCategoryRepository repository, Guid? currentId = null)
            : base(currentId)
        {
            _repository = repository;

            RuleFor(x => x.Code)
                .NotEmpty().WithMessage("Mã loại nhân sự không được bỏ trống.")
                ...
                .WithMessage("Mã loại nhân sự này đã tồn tại.");

            RuleFor(x => x.Name)
                .NotEmpty().WithMessage("Tên loại nhân sự không được bỏ trống.")
                ...
                .WithMessage("Tên loại nhân sự này đã tồn tại.");
        }
    }
}
